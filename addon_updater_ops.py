...
try:
    ...
    print("=" * 80)
    print("ERROR INITIALIZING UPDATER - FULL TRACEBACK:")
    print("=" * 80)
    print(str(e))
    print("=" * 80)
    traceback.print_exc()
    ...
    class SingletonUpdaterNone(object):
        ...
        def __init__(self):
            ...
            self.error = "Error initializing updater"
            self.error_msg = str(e) # Store actual error message
            self.async_checking = None
            # Add common properties that register() might try to set
            self.user = None
            self.repo = None
            self.website = None
            ...
            self.backup_current = True
            self.backup_ignore_patterns = None
            self.overwrite_patterns = []
            self.remove_pre_update_patterns = []
            self.include_branches = False
            self.use_releases = False
            self.include_branch_list = []
            self.manual_only = False
            self.fake_install = False
            self.show_popups = True
            self.version_min_update = None
            self.version_max_update = None
            self.skip_tag = None
            self.select_link = None
            self.auto_reload_post_update = False
            ...
            self.json = {}
            self._ignore_this_session = False
        ...
        def set_check_interval(self, **kwargs):
            pass
        
        def check_for_update_now(self, callback):
            pass
    ...
    # Don't override error_msg, keep the original exception info
...
class AddonUpdaterInstallPopup(bpy.types.Operator):
    ...
    clean_install = bpy.props.BoolProperty(
        ...
        default=True, # Changed to True for safer updates
        options={'HIDDEN'}
    )
    ...
    def draw(self, context):
        ...
        if updater.invalid_updater:
            ...
            row = col.row()
            row.prop(self, "clean_install") # Show clean install option
...
class AddonUpdaterIgnore(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if updater.invalid_updater:
            return {'CANCELLED'}
        updater.ignore_update()
        self.report({"INFO"}, "Update ignored for this session")
        return {'FINISHED'}
...
def updater_run_install_popup_handler(scene):
    ...
    if updater._ignore_this_session:
        return  # Don't do popup if ignored for this session
...
def post_update_callback(module_name, res=None):
    ...
    if res is None:
        ...
        # ✨ AUTO-RELOAD after successful update (SAME AS LOCAL INSTALL - NO RESTART!)
        try:
            print("\n" + "="*60)
            print("🔄 AUTO-RELOAD: Starting module reload...")
            print("="*60)
            
            addon_name = "Ultimate_Bak3"
            
            # Step 1: Reload modules (NO disable/enable needed!)
            print(f"[1/3] Reloading modules...")
            try:
                from .modules.update import core as update_core
                reloaded = update_core.reload_modules(addon_name)
                print(f"  ✓ Reloaded {reloaded} modules")
            except Exception as e:
            ...
            import time
            print(f"[2/3] Waiting for initialization...")
            time.sleep(1.0)
            
            # Step 3: Verify
            print(f"[3/3] Verifying...")
            try:
                from .modules.update import operators as update_ops
                is_ready, msg = update_ops.verify_addon_ready(addon_name)
            except:
                # Fallback verification
                is_ready = addon_name in sys.modules
                msg = "Basic check" if is_ready else "Not in sys.modules"
            
            print(f"  Status: {msg}")
            
            if is_ready:
                print("="*60)
                print("✅ AUTO-RELOAD SUCCESSFUL!")
                print("   No restart needed - addon ready to use!")
                print("="*60 + "\n")
            else:
                print("="*60)
                print("⚠ AUTO-RELOAD: Verification pending")
                print(f"   {msg}")
                print("   Features may need a moment to initialize")
                print("="*60 + "\n")
            ...
            try:
                for window in bpy.data.window_managers[0].windows:
                    for area in window.screen.areas:
                        area.tag_redraw()
                print("[UI] Refreshed all areas")
            except:
                pass
            
            # Step 5: Delayed reconfiguration (like Local Install)
                ...
                try:
                    if addon_name in sys.modules:
                        ...
                        if hasattr(main_mod, 'configure_updater'):
                            ...
                            print(f"[Config] Updater reconfigured")
                except:
                    pass
                return None
            ...
            import traceback
            traceback.print_exc()
            print("   Showing standard update popup...")
...
def update_notice_box_ui(self, context):
    ...
    # If user pressed ignore for this session, don't draw the box.
    if updater._ignore_this_session:
        return
    if "ignore" in updater.json and updater.json["ignore"]:
        return
...
def register(bl_info):
    ...
    # Reset JSON cache to force fresh check
    ...
    updater.use_releases = True  # User requested release assets
